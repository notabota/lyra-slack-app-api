...
export const reactionsCountRouter = createTRPCRouter({
  getList: publicProcedure
    .meta({ openapi: { method: 'GET', path: '/reactions-count' } })
    .input(z.object({
    ...
    .query(async ({ ctx, input }) => {
      ...
      // Get reaction counts grouped by userId
      ...
      const userIds = reactionCounts.map(m => m.userId);
      const users = await ctx.db.user.findMany({
        where: { id: { in: userIds } },
        select: { id: true, firstName: true, lastName: true }
      });

      // Get total number of unique users with reactions
      const total = await ctx.db.reaction.groupBy({
        by: ['userId'],
        where: dateFilter,
